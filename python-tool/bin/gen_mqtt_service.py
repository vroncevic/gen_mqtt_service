...
class gen_mqtt_service(CfgBase):
    """
        Define class gen_mqtt_service with attribute(s) and method(s).
    ...
    VERBOSE = 'gen_mqtt_service'
    __CONFIG = '/../conf/gen_mqtt_service.cfg'
    ...
    def __init__(self, verbose=False):
        """
            ...
            :exceptions: None
        """
        verbose_message(gen_mqtt_service.VERBOSE, verbose, 'Initial configuration')
        current_dir = Path(__file__).resolve().parent
        base_config_file = "{0}{1}".format(current_dir, gen_mqtt_service.__CONFIG)
        CfgBase.__init__(self, base_config_file, verbose=verbose)
        if self.tool_status:
            self.add_new_option(
                gen_mqtt_service.__OPS[0],
                gen_mqtt_service.__OPS[1],
                dest='option',
                help='Option tool'
            )
    ...
    def process(self, verbose=False):
        ...
        if self.tool_status:
            ...
            if num_of_args_sys > 1:
                option = sys.argv[1]
                if option not in gen_mqtt_service.__OPS:
                    sys.argv = []
                    sys.argv.append('-h')
            else:
                sys.argv.append('-h')
            ...
            error_message('gen_mqtt_service', 'Tool is not operational')
